<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-text>
            ...
            <CompanyId offline label="회사ID" v-model="value.companyId" :editMode="editMode" @change="change"/>
            <List&lt;SalesItems&gt; offline label="수주제품" v-model="value.salesItems" :editMode="editMode" @change="change"/>
            <SalesItemsManager offline label="수주제품" v-model="value.salesItems" :editMode="editMode" @change="change"/>
	...
	async created() {
        ...
        methods: {
            ...
            async updateSalesOrder(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['updatesalesorder'].href), params)
