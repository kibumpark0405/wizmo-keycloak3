<template>
    <v-card outlined>
        <v-card-title>
            SalesOrder # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>

        <v-card-text>
            <div>
                <String label="SalesOrderNumber" v-model="item.salesOrderNumber" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/salesOrders/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
